...
public class Oak extends Tree {
    public Oak(String name, int x, int y){
        super(name,x,y);
        this.seedCD1 = (ran.nextInt(4)+3)*21600;
        this.seedCD2 = seedCD1;
        this.growthTime = (ran.nextInt(2)+4)*21600;
        this.age = (ran.nextInt(3)+10)*21600;
    }
                        ...
                        for(int direct:huong){
                            ...
                            if(x+tmp1>=0 && x+tmp1<=499 && y+tmp2>=0 && y+tmp2<=499) {
                                if (toaDoSV[x + tmp1][y + tmp2] == null) {
                                    Oak tmp = EntityFactory.CreateEntity(Oak::new,name,x+tmp1,y+tmp2);
