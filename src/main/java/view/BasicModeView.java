...
public class BasicModeView {
    ...
    public BasicModeView(MapViewer manager) {
        this.manager = manager;
        this.worldMap = manager.getWorldMap();
        this.basicRenderer = new BasicRenderer(worldMap);

        this.scale = manager.getBasicViewScale();
        this.offsetX = manager.getViewOffsetX();
        this.offsetY = manager.getViewOffsetY();
        ...
        Button btnSwitchGraphicMode = new Button("🔄 Graphic View");
        btnSwitchGraphicMode.setMaxWidth(Double.MAX_VALUE);
        btnSwitchGraphicMode.setStyle("-fx-font-weight: bold; -fx-text-fill: #55d6ff; -fx-cursor: hand;");
        btnSwitchGraphicMode.setOnAction(e -> {
            double centerX = (-offsetX + canvas.getWidth() / 2.0) / scale;
            double centerY = (-offsetY + canvas.getHeight() / 2.0) / scale;
            manager.setViewCenter(centerX, centerY);
            manager.setBasicViewScale(scale);
            manager.setGraphicViewScale(scale / 32.0);
            manager.showGraphicMode();
        });

        Button btnBackToMenu = new Button("⬅ Exit to Main Menu");
        btnBackToMenu.setMaxWidth(Double.MAX_VALUE);
        btnBackToMenu.setStyle("-fx-font-weight: bold; -fx-text-fill: #ff4d4d; -fx-cursor: hand;");
        btnBackToMenu.setOnAction(e -> manager.showMainMenu());
        rightPanel.getChildren().addAll(btnSwitchGraphicMode, btnBackToMenu);
                ...
                if (gridX >= 0 && gridX < WorldMap.SIZE && gridY >= 0 && gridY < WorldMap.SIZE) {
                    ...
                    if (chunkMap != null && chunkMap[chunkY][chunkX] != null) {
                        ...
                        if ("SPAWN_RABBIT".equals(currentInteractionMode)) {
                            ...
                            if (manager.getSharedSimulationManager() != null) manager.getSharedSimulationManager().registerBrainForEntity(ent);
        ...
        Platform.runLater(() -> {
            if (manager != null && manager.getViewCenterX() >= 0) {
                double centerX = manager.getViewCenterX();
                double centerY = manager.getViewCenterY();
                this.scale = manager.getBasicViewScale();
                this.offsetX = (canvas.getWidth() / 2.0) - centerX * scale;
                this.offsetY = (canvas.getHeight() / 2.0) - centerY * scale;
                drawMap(canvas);
            }
        });
    ...
    private void registerBrainForEntity(Entity e) {
        if (e == null || !(e instanceof Animals)) return;
        Animals a = (Animals) e;
    ...
    private VBox createRightPanel() {
        ...
        Label lblSpeedTitle = new Label("SPEED CONTROLS");
        lblSpeedTitle.setFont(Font.font("Segoe UI", FontWeight.BOLD, 12));
        lblSpeedTitle.setTextFill(Color.LIGHTBLUE);

        javafx.scene.control.ChoiceBox<String> choiceSpeed = new javafx.scene.control.ChoiceBox<>();
        choiceSpeed.getItems().addAll("1x", "1.25x", "1.5x", "2x", "3x", "4x");
        choiceSpeed.setValue("1x");
        choiceSpeed.setMaxWidth(Double.MAX_VALUE);
        choiceSpeed.setOnAction(e -> {
            if (manager.getSharedSimulationManager() != null) {
                String selected = choiceSpeed.getValue();
                if (selected != null) {
                    try {
                        String numericPart = selected.replace("x", "");
                        double multiplier = Double.parseDouble(numericPart);
                        manager.getSharedSimulationManager().setSpeedMultiplier(multiplier);
                    } catch (Exception ignored) {}
                }
            }
        });

        Button btnSeasonJump = new Button("⏭ Season Jump");
        btnSeasonJump.setMaxWidth(Double.MAX_VALUE);
        btnSeasonJump.setStyle("-fx-background-color: #f39c12; -fx-text-fill: white;");
        btnSeasonJump.setOnAction(e -> {
            core.TimeSystem.jumpToLastDayOfPreviousSeasonAt2330();
            updateTimeInformation();
        });

        VBox speedBox = new VBox(6, lblSpeedTitle, choiceSpeed, btnSeasonJump);
        speedBox.setPadding(new Insets(6));
        speedBox.setStyle("-fx-background-color: #1e1e1e; -fx-background-radius: 4; -fx-border-color: #3a3a3a;");
    ...
    private void updateTimeInformation() {
        if (manager.getSharedSimulationManager() == null) return;
        try {
            ...
            if (lblClock != null) lblClock.setText("Time: Sync Error...");
        }
    }
    ...
    private void refreshSelectedAnimalDebug() {
        ...
        if (selectedAnimal == null || !selectedAnimal.checkAlive()) {
            ...
            basicRenderer.setSelectedAnimal(null);
            if (selectedAnimal == null && selectedChunk == null) {
                basicRenderer.setSelectedChunk(null);
            }
            return;
        }
        ...
        brain.controller.AnimalBrainUpdate brain = (manager.getSharedSimulationManager() != null) ?
                manager.getSharedSimulationManager().getBrainMap().get(selectedAnimal) : brainMap.get(selectedAnimal);
