...
public class MainController {
    ...
    @FXML
    private Label lblSelectedAnimalLeft;

    @FXML
    private Label lblSelectedChunkLeft;
    ...
    @FXML
    void onSwitchToBasicModeClicked(javafx.event.ActionEvent event) {
        System.out.println("Switch to Basic Mode button clicked.");
        if (view.MapViewer.instance != null) {
            if (mapController != null) {
                double centerX = (mapController.getCanvasWidth() / 2.0 - mapController.getOffsetX()) / (mapController.getTileSize() * mapController.getScale());
                double centerY = (mapController.getCanvasHeight() / 2.0 - mapController.getOffsetY()) / (mapController.getTileSize() * mapController.getScale());
                view.MapViewer.instance.setViewCenter(centerX, centerY);
                view.MapViewer.instance.setGraphicViewScale(mapController.getScale());
                view.MapViewer.instance.setBasicViewScale(mapController.getScale() * 32.0);
            }
            view.MapViewer.instance.showBasicMode();
        } else {
            System.err.println("❌ LỖI: MapViewer.instance đang bị null, không thể chuyển sang Basic Mode!");
        }
    }
    ...
    public brain.controller.SimulationManager setWorldMapContext(WorldMap realWorldMap, int gridSize) {
        ...
        if (mapController != null) {
            ...
            if (controlPanelController != null) {
                controlPanelController.setMapController(mapController);
                controlPanelController.refreshTimeStatus();
            }
            mapController.setControlPanel(controlPanelController);
    ...
    public void setSelectionInfo(String animalInfo, String chunkInfo) {
        if (lblSelectedAnimalLeft != null) {
            lblSelectedAnimalLeft.setText(animalInfo != null ? animalInfo : "Click an animal or a chunk to inspect it.");
        }
        if (lblSelectedChunkLeft != null) {
            lblSelectedChunkLeft.setText(chunkInfo != null ? chunkInfo : "Selected chunk info will appear here.");
        }
    }
}
